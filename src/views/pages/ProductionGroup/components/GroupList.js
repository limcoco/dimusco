...
import InstitutionGroupsModal from './InstitutionGroupsModal'
...
export default class GroupList extends React.Component {
  ...
  // Read List Group
  onRead = (search, page) => {
    const {getProductionGroups, match: { params: { prid } } } = this.props;
    getProductionGroups(this.onReadSuccess, this.onReadFailed, prid)
  }

  onReadSuccess(response) {
    this.onAddMemberGroup("administrator", true)
  ...
  onRegister(gid) {
    ...
    const {match: { params: { prid } } } = this.props;

    const payload =  {name: 'new group', production: prid }
    if (gid) {
      payload.parent = gid;
      delete payload.production;
    }
      
    this.props.createProductionGroup(this.onRegisterSuccess, this.onRegisterFailed, payload);
    this.setState({ toggleAddGroup: !this.state.toggleAddGroup, in_update: true })
  }
  ...
  orderGroup = () => {
    this.props.reorderGroup(this.state.dragged, this.state.dropped, this.onOrderGroupSuccess)
  }
    ...
    return (
      <div className="animated fadeIn">
        ...
        >
          ...
          <InstitutionGroupsModal
            words={words}
            {...this.props}
            getProdGroups={this.onRead}
          />
