...
class Product extends React.Component {
  state = {
    activeTab: 1,
    isLoading: true,
    keyQuery: qs.parse(this.props.location.search).key,
  ...
  onBuy = () => {
    ...
    if (PublisherReducer.is_auth) {
      ...
      this.toggleConfirmationModal(words['popup_login-required_small'], words['popup_login-required_big'])
      return
    }
  ...
  onReadDetail = (sid, id_curr) => {
    this.props.getProduct('get-score', { q: sid, cur: id_curr }, this.getToken(), this.getProductSuccess)
  }
  ...
  hasContract = ssid => {
    this.props.checkProductHasContact(ssid, this.getToken(), this.onHasContactSuccess)
  }

  onHasContactSuccess = (response) => {
    this.setState(response)
  }

  render() {
    const { infoMsg, showConfirmationModal, headline, sid, contract, keyQuery } = this.state
    const {
      ActiveLanguageReducer: { words },
      previewMode,
      togglePreviewMode,
