...
class Layer extends Component {
  ...
  onRegisterLayer = () => {
    ...
    let payload = {
      score: items[activeScoreIndex].sid,
      name: "New Layer",
      production: prid
    }
  ...
  onReadLayer = (purchase_id) => {
    ...
    Request(
      ...
      "read-layer",
      { Authorization: "Token " + this.token },
      { assignment: new_aid, production: prid },
      [],
  ...
  onReadAssignmentSuccess = (response) => {
    ...
    if (loading_proccess.type === "group_assign") {
      if (groups.length === response.data.assigned.viewers.length) {
        this.onReadAssignment()
      }

    } else if (loading_proccess.type === "group_unassign") {
      if (unregisteredGroups.length === response.data.unassigned.viewers.length) {
        this.onReadAssignment()
      }

    } else if (loading_proccess.type === "member_assign") {
      if (members.length === response.data.assigned.masters.length) {
        this.onReadAssignment()
      }

    } else if (loading_proccess.type === "member_unassign") {
      if (unregisteredMembers.length === response.data.unassigned.masters.length) {
        this.onReadAssignment()
      }
    }
    this.setState({
      groups: this.sortData(response.data.assigned.viewers),
      members: this.sortData(response.data.assigned.masters),
      unregisteredGroups: this.sortData(response.data.unassigned.viewers),
      unregisteredMembers: this.sortData(response.data.unassigned.masters),
      loading_proccess: {
        type: null,
        index: -1
      }
  ...
  onUnassign = (index, data) => {
    const { activeLayerIndex, groups, members, itemsLayer } = this.state
    let url, id = null
    if (data.type === "group") {
      url = "layer-unassign-group"
      id = groups[index].laid
      data['laid'] = id
      this.setState({
        loading_proccess: {
          type: "group_unassign",
      ...
      if (layer) {
        return (
          <LayerRow
            ...
            newId={this.newId}
            newLayerLabel={words['gen_layer_new-layer'] || 'gen_layer_new-layer'}
          />
        )
      } else {
        return (
  ...
  render() {
    ...
    const { words } = this.props.ActiveLanguageReducer;
    return (
      <section className="product-content no-border-bottom production-layers">
        <div className="container">
          {/* TITLE */}
          <div className="musinote-center institution-name-header">
            <h3 className="no-margin full-width">
              <small>{words.prod_assign_layers}</small>
            </h3>
          </div>
          {/* END TITLE */}
